...
type ContextProject = { id: string; title: string; batchCount: number; latestBatchLabel: string };
type ContextDispatchDraft = { entryId: string; materialId: string; materialTitle: string; eventTitle: string; candidateKeys: string[]; candidates: ContextCandidate[]; projects: ContextProject[] };
...
function DispatchBadges({ entry }: { entry: EnglishImageEntry }) {
  ...
  return <div className="english-image-destinations"><span>{entry.attachments.length} 張圖片</span>{entry.status === "organized" && <span className="is-organized">✓ 已完成整理</span>}<span>{entry.vocabForgeExports.length ? `VF 已同步 ${entry.vocabForgeExports.length} 字` : "VF 尚未派送"}</span>{entry.contextRoomStatus === "idle" || !entry.contextRoomUrl ? <span>語境尚未派送</span> : <a href={entry.contextRoomUrl} target="_blank" rel="noreferrer">{entry.contextRoomStatus === "synced" ? `語境第 ${entry.contextRoomExport?.batchPosition ?? 1} 批 ↗` : "語境待接續 ↗"}</a>}{failures > 0 && <span className="is-error">同步失敗 {failures} 字</span>}</div>;
}
...
export default function EnglishImageInbox() {
  ...
  async function openContextDispatch(entry: EnglishImageEntry) {
    setBusy(entry.id); setError("");
    try { const result = await json<{ candidates: ContextCandidate[]; projects: ContextProject[]; defaults: { materialId: string; materialTitle: string; eventTitle: string } }>(await fetch(`/api/dojo/english-images/context-room?id=${encodeURIComponent(entry.id)}`, { cache: "no-store" })); const selectedProject = result.projects.find((item) => item.id === result.defaults.materialId); setContextDraft({ entryId: entry.id, materialId: selectedProject?.id || "", materialTitle: selectedProject?.title || result.defaults.materialTitle, eventTitle: result.defaults.eventTitle, candidateKeys: result.candidates.map((item) => item.key).slice(0, 3), candidates: result.candidates, projects: result.projects }); setVocabDraft(null); }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); } finally { setBusy(null); }
  }
  ...
  async function sendContextDispatch() {
    ...
    try { const result = await json<{ entry: EnglishImageEntry }>(await fetch("/api/dojo/english-images/context-room", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: contextDraft.entryId, materialId: contextDraft.materialId, materialTitle: contextDraft.materialTitle, eventTitle: contextDraft.eventTitle, candidateKeys: contextDraft.candidateKeys }) })); replaceEntries([result.entry]); setCompletionPromptId(result.entry.id); setContextDraft(null); }
    catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); } finally { setBusy(null); }
  }
  ...
  function renderContextDispatch(entry: EnglishImageEntry) {
    ...
    return <div className="english-image-context-dispatch"><div className="english-image-context-head"><div><small>語境修習室</small><h5>{form.heading}</h5></div><button className="text-link" onClick={() => setContextDraft(null)}>關閉</button></div><label>加入方式<select className="field" value={contextDraft.materialId || "__new__"} onChange={(event) => { const materialId = event.target.value === "__new__" ? "" : event.target.value; const project = contextDraft.projects.find((item) => item.id === materialId); setContextDraft({ ...contextDraft, materialId, materialTitle: project?.title || contextDraft.materialTitle }); }}><option value="__new__">建立新的素材專案</option>{contextDraft.projects.map((project) => <option key={project.id} value={project.id}>加入「{project.title}」（{project.batchCount} 批）</option>)}</select></label>{contextDraft.materialId && (() => { const project = contextDraft.projects.find((item) => item.id === contextDraft.materialId); return <p className="muted-note">將建立第 {(project?.batchCount ?? 0) + 1} 批{project?.latestBatchLabel ? `；上一批是「${project.latestBatchLabel}」` : ""}。</p>; })()}<label>{form.materialLabel}<input className="field" value={contextDraft.materialTitle} disabled={Boolean(contextDraft.materialId)} onChange={(event) => setContextDraft({ ...contextDraft, materialTitle: event.target.value })} placeholder={form.materialPlaceholder} /></label><label>{form.eventLabel}<input className="field" value={contextDraft.eventTitle} onChange={(event) => setContextDraft({ ...contextDraft, eventTitle: event.target.value })} placeholder={form.eventPlaceholder} /></label><fieldset><legend>選擇真正想留下的表達（最多 5 項）</legend>{contextDraft.candidates.length === 0 ? <p className="muted-note">目前沒有可派送的片語、句型或用法。</p> : contextDraft.candidates.map((candidate) => { const checked = contextDraft.candidateKeys.includes(candidate.key); return <label className="english-image-context-choice" key={candidate.key}><input type="checkbox" checked={checked} disabled={!checked && contextDraft.candidateKeys.length >= 5} onChange={(event) => setContextDraft({ ...contextDraft, candidateKeys: event.target.checked ? [...contextDraft.candidateKeys, candidate.key] : contextDraft.candidateKeys.filter((key) => key !== candidate.key) })} /><span><small>{CONTEXT_KIND_LABEL[candidate.kind]}</small><b>{candidate.text}</b>{candidate.meaning && <em>{candidate.meaning}</em>}{candidate.usage && <i>{candidate.usage}</i>}</span></label>; })}</fieldset><p className="muted-note">未勾選的 AI 建議仍留在原始影像紀錄。</p><button className="primary" disabled={busy === entry.id || !contextDraft.materialTitle.trim() || !contextDraft.eventTitle.trim()} onClick={() => void sendContextDispatch()}>{busy === entry.id ? "派送中…" : contextDraft.materialId ? "加入既有專案，建立新批次" : `建立新專案並留下 ${contextDraft.candidateKeys.length} 項表達`}</button></div>;
  }
  ...
  function renderDetail(entry: EnglishImageEntry) {
    ...
    return <section id="english-image-detail" className="english-image-detail"><div className="english-image-detail-nav"><button onClick={closeEntry}>← 返回精簡清單</button><span>{entry.status === "organized" ? "已完成素材" : "待處理素材"}</span></div><header className="english-image-detail-head"><div><small>{englishImageRouteLabel(entry.route)}・{entry.capturedAt.slice(0, 10)}</small><h5>{entry.title}</h5><p>{englishImageStageLabel(entry)}</p></div><span className={`analysis-${entry.analysisStatus}`}>{analysisLabel(entry)}</span></header><DispatchBadges entry={entry} />{deepLinkMessage && <p className="english-image-deep-link-note">{deepLinkMessage}</p>}{notice && <p className="form-success">{notice}</p>}{error && <p className="form-error">{error}</p>}{entry.analysisError && <p className="form-error">{entry.analysisError}</p>}
      ...
      <details className="english-image-detail-section english-image-dispatch-section" open={contextDraft?.entryId === entry.id}><summary>語境修習室派送 <span>{entry.contextRoomStatus === "synced" ? `已送第 ${entry.contextRoomExport?.batchPosition ?? 1} 批` : "尚未派送"}</span></summary><div className="english-image-section-body"><button disabled={busy === entry.id || entry.route === "pending" || entry.analysisStatus === "idle"} onClick={() => void openContextDispatch(entry)}>{busy === entry.id ? "讀取中…" : entry.contextRoomStatus === "synced" ? "查看語境專案／再次派送" : entry.route === "reading" ? "加入閱讀專案／新批次" : "加入語境專案／新批次"}</button>{renderContextDispatch(entry)}</div></details>
