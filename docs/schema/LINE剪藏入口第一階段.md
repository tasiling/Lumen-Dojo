...
單獨傳入 LINE 的圖片預設寫入 `行光英文影像-` 紀錄，並顯示於野採的英文影像匣。使用者可在 LINE 選擇「遊戲英文」、「英文日常」或「一般剪藏」；前兩者保留獨立英文影像資料，後者才把同一個 Notion 頁面轉為 `行光捕捉-`，因此原圖不會遺失或重複保存。英文影像與修習所之間的資料連結保留，但修習所不直接呈現整個影像匣。
